...
success_criteria:
  ...
  - criterion: "Stream E / MatchCard renders tier via `tierForScore` (single source of truth in MatchScoreChip.tsx, which holds the 4 ratified tier literals); shareMatchCard util exists and imports react-native-view-shot."
    threshold: hard
    verify_command: "node -e \"const fs=require('fs'); if(!fs.existsSync('components/MatchCard.tsx')){console.error('FAIL: components/MatchCard.tsx missing');process.exit(1)} if(!fs.existsSync('utils/shareMatchCard.ts')){console.error('FAIL: utils/shareMatchCard.ts missing');process.exit(1)} const chip=fs.readFileSync('components/MatchScoreChip.tsx','utf8'); const tiers=['Getting There','In Sync','Tight Loop','Soulmates']; const chipHits=tiers.filter(t=>chip.includes(t)); if(chipHits.length<4){console.error('FAIL: MatchScoreChip missing tier labels; present:',chipHits.join(', '));process.exit(1)} const mc=fs.readFileSync('components/MatchCard.tsx','utf8'); if(!/tierForScore/.test(mc)){console.error('FAIL: MatchCard must use tierForScore from MatchScoreChip (single source of truth for tier labels)');process.exit(1)} const sh=fs.readFileSync('utils/shareMatchCard.ts','utf8'); if(!/react-native-view-shot/.test(sh)){console.error('FAIL: shareMatchCard.ts must import react-native-view-shot');process.exit(1)} console.log('OK')\""
